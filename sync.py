...
while 1:
    ...
    if dst_arr:  # if source is not new
        for path_src in src_arr:
            ...
            for path_dst in dst_arr:
                ...
                if path_src == path_dst:  # sort pathes and if the new file is found - rewrite
            ...
            if count == 0:  # if count=0  - new path. Create a folder or a file
                if os.path.isfile(full_path_src):
        ...
        for path_dst in dst_arr:  # determine array with pathes which is needed to delete from source
            flag = False
            for path_src in src_arr:
        ...
        for i in range(len(del_arr), 0, -1):  # delete folders and files from source
